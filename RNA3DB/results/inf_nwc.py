...
# Output CSV file
output_csv = 'interaction_nWC.csv'
...
for csv_file in csv_files:
    ...
    # Extract inf_WC values and update the row if conditions are met
    if df['fn'].str.contains('_df').any():
        row['df'] = df.loc[df['fn'].str.contains('_df'), 'inf_nWC'].values[0]

    if df['fn'].str.contains('_drfold').any():
        row['drfold'] = df.loc[df['fn'].str.contains('_drfold'), 'inf_nWC'].values[0]

    if df['fn'].str.contains('_rf').any():
        row['rf'] = df.loc[df['fn'].str.contains('_rf'), 'inf_nWC'].values[0]

    if df['fn'].str.contains('_rhofold').any():
        row['rhofold'] = df.loc[df['fn'].str.contains('_rhofold'), 'inf_nWC'].values[0]

    if df['fn'].str.contains('_rnac').any():
        row['rnac'] = df.loc[df['fn'].str.contains('_rnac'), 'inf_nWC'].values[0]

    if df['fn'].str.contains('_trr').any():
        row['trr'] = df.loc[df['fn'].str.contains('_trr'), 'inf_nWC'].values[0]

    if df['fn'].str.contains('_3drna').any():
        row['3drna'] = df.loc[df['fn'].str.contains('_3drna'), 'inf_nWC'].values[0]
