...
for csv_file in csv_files:
    ...
    if df['fn'].str.contains('_df').any():
        row['df'] = df.loc[df['fn'].str.contains('_df'), 'inf_WC'].values[0]

    if df['fn'].str.contains('_drfold').any():
        row['drfold'] = df.loc[df['fn'].str.contains('_drfold'), 'inf_WC'].values[0]

    if df['fn'].str.contains('_rf').any():
        row['rf'] = df.loc[df['fn'].str.contains('_rf'), 'inf_WC'].values[0]

    if df['fn'].str.contains('_rhofold').any():
        row['rhofold'] = df.loc[df['fn'].str.contains('_rhofold'), 'inf_WC'].values[0]

    if df['fn'].str.contains('_rnac').any():
        row['rnac'] = df.loc[df['fn'].str.contains('_rnac'), 'inf_WC'].values[0]

    if df['fn'].str.contains('_trr').any():
        row['trr'] = df.loc[df['fn'].str.contains('_trr'), 'inf_WC'].values[0]

    if df['fn'].str.contains('_3drna').any():
        row['3drna'] = df.loc[df['fn'].str.contains('_3drna'), 'inf_WC'].values[0]
