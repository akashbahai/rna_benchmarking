...
while IFS= read -r target || [[ -n "$target" ]]; do
    ...
    pdb_tidy "${target_native_pdb}" > "${target}/${target}_native_tidy.pdb"
    pdb_fixinsert "${target}/${target}_native_tidy.pdb" > "${target}/${target}_native_temp.pdb"
    
    
    #preprocess native pdb files
    python preprocessing_latest.py "${target}/${target}_native_temp.pdb" "${fastafile}"
    processed_native_pdb="${target}/${target}_native_remapped.pdb"

    #pdb_tidy "${processed_native_pdb}" > "${target}/${target}_native_tidy.pdb" 

    #pdb_selaltloc "${target}/${target}_native_tidy.pdb"  > "${target}/${target}_native_altloc.pdb"
    #fix missing atoms in the native pdb file
    pdbfixer "${processed_native_pdb}" --output "${target}/${target}_native_fixatom.pdb" --add-atoms heavy
    ...
    rna_pdb_tools.py --rpr --fix "${target}/${target}_native_fixatom.pdb" > "${final_native_file}"


    rm "${target}/${target}_native_tidy.pdb"
    rm "${target}/${target}_native_temp.pdb"
    #rm "${target}/${target}_native_altloc.pdb"
    rm "${target}/${target}_native_fixatom.pdb"
